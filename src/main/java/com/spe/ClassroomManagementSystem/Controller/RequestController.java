...
public class RequestController {
    ...
    @RequestMapping("/postRequest/{classCode}/{date}")
    public RedirectView classroomRequest(HttpSession session,
                                         @PathVariable String classCode,
                                         @PathVariable Date date,
                                         HttpServletRequest request) {
        logger.trace("classroomRequest called");
        boolean success_requested = requestService.saveRequest(session, classCode, date);
...
//        String reqStartTime = (String) session.getAttribute("reqStartTime");
//        String reqEndTime = (String) session.getAttribute("reqEndTime");
//        String cleaningNeeded = (String) session.getAttribute("cleaningNeeded");
//        String projectorNeeded = (String) session.getAttribute("projectorNeeded");
//        String reqPlugs = (String) session.getAttribute("reqPlugs");
//        String reqDate = (String) session.getAttribute("reqDate");
//        String endDate = (String) session.getAttribute("endDate");
//        String reqCapacity = (String) session.getAttribute("reqCapacity");
        
        
        RedirectAttributes redirectAttributes = new RedirectAttributesModelMap();    
        redirectAttributes.addFlashAttribute("purpose", session.getAttribute("purpose"));
        redirectAttributes.addFlashAttribute("startTimeFormat", session.getAttribute("startTimeFormat"));
        redirectAttributes.addFlashAttribute("endTimeFormat", session.getAttribute("endTimeFormat"));
        redirectAttributes.addFlashAttribute("cleaningNeeded", session.getAttribute("cleaningNeeded"));
        redirectAttributes.addFlashAttribute("projectorNeeded", session.getAttribute("projectorNeeded"));
        redirectAttributes.addFlashAttribute("plugs", session.getAttribute("plugs"));
        redirectAttributes.addFlashAttribute("startDate", session.getAttribute("startDate"));
        redirectAttributes.addFlashAttribute("endDate", session.getAttribute("endDate"));
        redirectAttributes.addFlashAttribute("capacity", session.getAttribute("capacity"));
        String uri = request.getScheme()+"://"+request.getServerName();
        if(Objects.nonNull(request.getServerPort()) && request.getServerPort()!= 0){
            uri += ":"+ request.getServerPort();
        }

        switch ((String) session.getAttribute("userType")) {
            case "professor":
//                rv.setUrl("/redirectAvailableClasses");
                rv.setUrl(uri+ "/redirectAvailableClasses");
                break;
            case "ta":
