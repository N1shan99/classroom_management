...
<%@ page import="com.spe.ClassroomManagementSystem.Models.Internship" %>
...
<%
  /*   List<ReturnableRequest> returnableRequestList = (List<ReturnableRequest>) session.getAttribute("timetableList");
    if (returnableRequestList.size() >0){ */
    	...
    	if(Objects.nonNull( session.getAttribute("internshipList"))){
    		internshipList = (List<Internship>) session.getAttribute("internshipList");
    	}
...
<div class="container" >
    ...
    <h2 >Hey, here are the Internship requests</h2>
    ...
    <table class="table table-hover table-responsive table-bordered" style="background: white">
        ...
        <c:forEach var="e" items="${internshipList}">
            <tr>
                <td>${e.internname}</td>
